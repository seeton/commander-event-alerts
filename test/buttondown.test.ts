...
import { monthlyMarkdown, sendMonthlyNewsletter } from "../src/buttondown";
...
describe("monthly delivery recovery", () => {
  ...
  it("does not mistake the previous month's email for the current month", async () => {
    mock(response({results:[{...draft,slug:'commander-events-2026-09',status:'sent'}],next:null}),response(draft),response(draft),response({...draft,status:'about_to_send'}));
    expect((await send()).duplicate).toBe(false);
  });

  it("follows pagination using the fixed provider host, not a returned URL", async () => {
    const sent={...draft,status:'sent'};
    const fetcher=mock(response({results:[],next:'https://untrusted.example/?page=2'}),response({results:[sent],next:null}),response(sent));
    expect((await send()).duplicate).toBe(true);
    expect(fetcher.mock.calls[1]![0]).toContain('https://api.buttondown.com/v1/emails?');
    expect(fetcher.mock.calls[1]![0]).toContain('page=2');
  });

  it("stops on a review rejection and never logs the private provider response", async () => {
    mock(response({code:'account_disabled',detail:'private@example.com'},403));
    await expect(send()).rejects.toThrow('Buttondown HTTP 403: account_disabled');
  });

  it("does not publish when the live response refers to another month", async () => {
    const fetcher=mock(response({results:[draft],next:null}),response({...draft,slug:'commander-events-2026-09'}));
    await expect(send()).rejects.toThrow('unexpected email slug');
    expect(fetcher).toHaveBeenCalledTimes(2);
  });
});
