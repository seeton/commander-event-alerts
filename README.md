...
オフで実行すると、その時点で開催前の対象イベントをまとめてメールします。同じイベントでも開催前であれば月初ごとに再通知します。例えば10月10日のイベントが8月に発表済みなら、8月・9月・10月の月初に届きます。定期確認は毎月1日 09:15 JST です。
...
外部パッケージや有料APIは使いません。公式ページを取得して固有イベント名で絞り込み、開催日が過ぎたイベントを除外します。メール送信にはResendの無料APIを使います。
